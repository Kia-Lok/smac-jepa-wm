...
"""
Train SMAC-JEPA with Markov recursive rollout loss + optional gradient deconflicting.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Detach pred_latent before decoded/presence auxiliary heads. "
            "This makes decoded_loss train decoder only and presence_loss train presence head only, "
            "instead of letting auxiliary losses update the rollout predictor."
        ),
    )
...
def markov_rollout_losses(
    ...
    *,
    ...
    # Optional deconflicting:
    # If detach_aux_losses=True, decoded_loss and presence_loss still train their heads,
    # but they do NOT backprop through pred_latent into the predictor/encoder rollout path.
    pred_latent_aux = pred_latent.detach() if detach_aux_losses else pred_latent

    decoded = model.decode_entities(pred_latent_aux.reshape(bsz * p * h, entities, latent_dim))
    ...
    presence_logits = model.predict_presence(pred_latent_aux.reshape(bsz * p * h, entities, latent_dim))
...
def main() -> None:
    ...
    saved_config = vars(args) | arch | {
        ...
        "training_regime": "markov_rollout_deconflict",
    ...
    print(
        ...
        f"detach_targets={args.detach_rollout_targets} detach_aux={args.detach_aux_losses} "
        ...
        for batch in loader:
            ...
            with autocast_context:
                losses = markov_rollout_losses(
                    ...
                    detach_aux_losses=args.detach_aux_losses,
