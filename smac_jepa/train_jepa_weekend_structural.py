...
def markov_rollout_rnn_losses(
    ...
    *,
    ...
    if dropout_probability > 0.0:
        ...
        max_agents = int(action_mask_seq.shape[-1]) if "action_mask_seq" in batch else 0
...
def main() -> None:
    ...
    if args.direct_action_fusion:
        ...
        memory_module.action_fusion_module = action_fusion_module
    ...
    if args.resume:
        ...
        if action_fusion_module is not None:
            if "action_fusion_module_state" not in checkpoint:
                raise SystemExit(
                    "Cannot resume direct-action-fusion run without "
                    "action_fusion_module_state"
                )
            action_fusion_module.load_state_dict(
                checkpoint["action_fusion_module_state"]
            )
