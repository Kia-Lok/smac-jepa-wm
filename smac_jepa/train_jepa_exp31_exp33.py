...
Run as:
    python -m smac_jepa.train_jepa_exp17_exp19_fixed
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SMAC-JEPA Exp31-Exp33 belief/planning experiments")
    ...
    parser.add_argument(
        ...
        default=250,
        help=(
            "Write checkpoint_recovery.pt after this many successful optimizer "
            "updates. 0 disables periodic recovery checkpoints."
        ),
    )
    parser.add_argument(
        "--max-consecutive-nonfinite-grad-steps",
        type=int,
        default=20,
        help=(
            "Abort only after this many consecutive optimizer updates have "
            "non-finite gradients. Individual bad AMP steps are skipped."
        ),
    )
    parser.add_argument(
        "--max-total-nonfinite-grad-steps",
        type=int,
        default=100,
        help=(
            "Abort after this many total non-finite-gradient steps in one run. "
            "A recovery checkpoint is written before aborting."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "AMP precision. auto uses BF16 when supported, otherwise FP16. "
            "BF16 has a much larger exponent range and is preferred for stability."
        ),
    )
    parser.add_argument(
        "--amp-init-scale",
        type=float,
        default=1024.0,
        help="Initial FP16 GradScaler scale. Lower than PyTorch's default to avoid early overflow.",
    )
    parser.add_argument(
        "--amp-growth-interval",
        type=int,
        default=10000,
        help="Successful FP16 steps required before increasing the GradScaler scale.",
    )
    parser.add_argument(
        "--amp-fallback-after-nonfinite",
        type=int,
        default=3,
        help=(
            "After this many consecutive non-finite-gradient steps, disable AMP "
            "for the rest of the run and continue in FP32. 0 disables fallback."
        ),
    )
    parser.add_argument(
        "--nonfinite-lr-backoff",
        type=float,
        default=0.5,
        help="Learning-rate multiplier applied when precision falls back to FP32.",
    )
    parser.add_argument(
        "--min-learning-rate",
        type=float,
        default=1e-6,
        help="Lower bound for automatic learning-rate backoff.",
    )
    parser.add_argument(
        ...
        default=2000,
        help=(
            "Linearly warm up posterior Barlow, memory Barlow, and SIGReg over "
            "this many successful optimizer updates to prevent early gradient spikes."
        ),
    )
    parser.add_argument(
        "--adam-eps",
        type=float,
        default=1e-6,
        help="AdamW epsilon. A larger value than 1e-8 improves numerical stability.",
    )
    ...
    # Weekend structural suite.
    parser.add_argument(
        "--residual-state-decoder",
        action=argparse.BooleanOptionalAction,
        default=False,
        help=(
            "Interpret the native decoder output as a state delta relative to "
            "the rollout-start entity state."
        ),
    )
    parser.add_argument(
        "--delta-loss-weight",
        ...
        default=0.0,
        help=(
            "Auxiliary meaningful-dynamic delta-state reconstruction loss. "
            "Zero disables it."
        ),
    )
    parser.add_argument(
        "--event-dynamics-weight",
        type=float,
        default=0.0,
        help=(
            "Additional multiplier applied only to latent dynamics and one-step "
            "losses for targets containing meaningful dynamic changes."
        ),
    )
    parser.add_argument(
        "--event-dynamics-threshold",
        type=float,
        default=0.01,
        help="Dynamic-coordinate threshold used by event-weighted dynamics.",
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            "Add a learned ordered joint-action residual directly to each "
            "predicted entity latent."
        ),
    )
    parser.add_argument(
        "--direct-action-hidden-dim",
        type=int,
        default=256,
    )
    parser.add_argument(
        "--enemy-observation-dropout",
        type=float,
        default=0.0,
        help=(
            "Probability of deliberately hiding each currently visible enemy "
            "observation during training. Evaluation remains unmodified."
        ),
    )
    parser.add_argument(
        "--hidden-reconstruction-weight",
        type=float,
        default=0.0,
        help=(
            "Additional decoded-state loss on deliberately hidden enemy target "
            "entities."
        ),
    )

    # Exp31--Exp33: controlled belief and planning-readiness training.
    parser.add_argument(
        "--init-from",
        default=None,
        help=(
            "Initialize model/memory/projector weights from a completed "
            "checkpoint without restoring optimizer, scheduler, epoch, or "
            "experiment configuration. Mutually exclusive with --resume."
        ),
    )
    parser.add_argument(
        "--occlusion-mode",
        choices=["none", "independent", "contiguous"],
        default="none",
        help=(
            "none: no artificial masking; independent: Exp25-style random "
            "visible-enemy masking; contiguous: hide one or more enemy spans "
            "after a visible observation."
        ),
    )
    parser.add_argument(
        "--contiguous-occlusion-spans",
        type=int,
        nargs="+",
        default=[1, 3, 5],
        help="Candidate contiguous hidden-span lengths.",
    )
    parser.add_argument(
        "--occlusion-spans-per-sample",
        type=int,
        default=1,
        help="Maximum non-overlapping contiguous spans selected per sample.",
    )
    parser.add_argument(
        "--last-seen-anchor-weight",
        type=float,
        default=0.0,
        help=(
            "Extra loss retaining last-observed values on deliberately hidden "
            "dynamic coordinates that truly remained unchanged."
        ),
    )
    parser.add_argument(
        "--last-seen-change-threshold",
        type=float,
        default=0.01,
    )
    parser.add_argument(
        "--hidden-presence-weight",
        type=float,
        default=0.0,
        help=(
            "Extra presence BCE over scheduled hidden spans, including target "
            "absence/death when present in the full-state target."
        ),
    )
    parser.add_argument(
        "--reappearance-consistency-weight",
        type=float,
        default=0.0,
        help=(
            "Extra decoded-state loss at the first originally visible frame "
            "after a contiguous hidden span."
        ),
    )
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Per-agent factual inverse-dynamics auxiliary used by Exp33. "
            "This is not a shuffled-action sensitivity objective."
        ),
    )
    parser.add_argument(
...
def resolve_amp_policy(
    *,
    ...
    """
    if not requested_amp or device.type != "cuda":
    ...
    if requested_dtype == "bf16":
        if not bf16_supported:
            raise SystemExit(
                "--amp-dtype bf16 was requested, but this CUDA device/PyTorch "
                "build does not report BF16 support."
            )
    ...
    if bf16_supported:
        return True, torch.bfloat16, False, "bf16(auto)"

    return True, torch.float16, True, "fp16(auto)"


def nonfinite_batch_fields(
    batch: dict[str, torch.Tensor],
    *,
    limit: int = 20,
) -> list[str]:
    """Find floating-point batch tensors containing NaN or Inf."""
    bad: list[str] = []
...
def apply_learning_rate_backoff(
    ...
    *,
    ...
    """Back off both current and scheduler base learning rates."""
    if not 0.0 < float(factor) <= 1.0:
        raise ValueError("Learning-rate backoff factor must be in (0, 1].")

    old_lr = float(optimizer.param_groups[0]["lr"])
    new_base_lrs: list[float] = []

    for base_lr in scheduler.base_lrs:
        new_base_lrs.append(max(float(base_lr) * float(factor), float(minimum)))
    scheduler.base_lrs = new_base_lrs

    for group in optimizer.param_groups:
        group["lr"] = max(float(group["lr"]) * float(factor), float(minimum))
...
class DirectOrderedActionFusion(nn.Module):
    """
    Agent-identity-preserving joint-action residual.

    The ordered one-hot joint action is compressed to one context vector.
    A learned entity embedding then lets the same joint action affect each
    entity differently. The final layer is zero-initialized so enabling the
    module starts exactly at the pre-existing predictor behaviour.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.action_net = nn.Sequential(
            nn.Linear(self.max_agents * self.n_actions, self.hidden_dim),
            nn.SiLU(),
        )
        entity_dim = min(64, self.hidden_dim)
        self.entity_embedding = nn.Embedding(self.max_entities, entity_dim)
    ...
    def forward(
        ...
        *,
        ...
        # action: [B, A, K] one-hot or [B, A] integer IDs
        if action.dim() == 2:
            action = F.one_hot(
                action.long().clamp(min=0, max=self.n_actions - 1),
            ...
            raise RuntimeError(
                f"Unexpected action shape for direct fusion: {tuple(action.shape)}"
            )

        batch, agents = action.shape[:2]
        if agents > self.max_agents:
            raise RuntimeError(
                f"actions contain {agents} agents but max_agents={self.max_agents}"
            )

        action = action * action_mask.to(dtype).unsqueeze(-1)
        if agents < self.max_agents:
            action = F.pad(
                action,
                (0, 0, 0, self.max_agents - agents),
            )

        context = self.action_net(action.reshape(batch, -1))
        ...
        if entities > self.max_entities:
            raise RuntimeError(
                f"entities={entities} exceeds max_entities={self.max_entities}"
            )
        entity_embed = self.entity_embedding(entity_ids)
        context = context[:, None, :].expand(-1, entities, -1)
        entity_embed = entity_embed[None, :, :].expand(batch, -1, -1)
        return self.output(torch.cat([context, entity_embed], dim=-1)).to(dtype)
...
def nonfinite_gradient_names(
    ...
    *,
    ...
    """Return a short diagnostic list of parameters with NaN/Inf gradients."""
    bad: list[str] = []
...
class PerAgentInverseDynamicsHead(nn.Module):
    """Predict each ally's factual action from its current and predicted next latent."""
    ...
    def forward(
        ...
        return self.net(
            torch.cat([current_latent, predicted_next_latent], dim=-1)
        )


def gather_rollout_sequence(
    sequence: torch.Tensor,
    rollout_window: int,
    rollout_horizon: int,
) -> torch.Tensor:
    """Gather target times start+h+1 from a [B,T,...] sequence."""
    ...
    if int(index.max().item()) >= int(sequence.shape[1]):
        raise RuntimeError(
            "Rollout target index exceeds sequence length: "
            f"max={int(index.max().item())} length={sequence.shape[1]}"
        )
...
def last_seen_before_sequence(
    ...
    """Last observed state, validity, and age immediately before every time."""
    ...
    return (
        ...
        torch.stack(ages, dim=1),
    )


def build_contiguous_occlusion_schedule(
    observation_mask_seq: torch.Tensor,
    entity_slot_mask_seq: torch.Tensor,
    ...
    *,
    ...
    """
    Select non-overlapping enemy spans after a visible state.

    A candidate must begin on an originally visible enemy and have a visible
    immediately preceding state. The remainder of the span may include natural
    occlusion or target disappearance, allowing the hidden-presence objective to
    see realistic continuations. A reappearance marker is emitted when the first
    frame after the span is originally visible.
    """
    observation = observation_mask_seq.bool()
    structural = (
        entity_slot_mask_seq.bool()
        & state_mask.bool().unsqueeze(-1)
    )
    enemy = torch.zeros_like(structural)
    enemy[..., int(max_agents):] = True
    structural &= enemy

    batch, time_steps, entities = observation.shape
    selected = torch.zeros_like(observation)
    reappearance = torch.zeros_like(observation)
    lengths = sorted({int(value) for value in spans if int(value) > 0})
    if not lengths:
        raise ValueError("contiguous occlusion requires positive spans")

    inf = torch.tensor(float("inf"), device=observation.device)
    batch_index = torch.arange(batch, device=observation.device)

    for _ in range(max(int(spans_per_sample), 0)):
        score_by_length: list[torch.Tensor] = []
        for length in lengths:
            scores = torch.full(
                (batch, time_steps, entities),
                float("inf"),
                device=observation.device,
                dtype=torch.float32,
            )
            if length >= time_steps:
                score_by_length.append(scores)
                continue

            # Starts are 1..T-L, giving T-L candidates.
            structural_window = (
                structural[:, 1:]
                .unfold(1, length, 1)
                .all(dim=-1)
            ...
            prior_visible = observation[:, : time_steps - length]
            start_visible = observation[:, 1 : time_steps - length + 1]
            valid = (
                structural_window
                & unused_window
                & prior_visible
            ...
            # Prefer spans followed by a true observation, without requiring it.
            after_visible = torch.zeros_like(valid)
            if time_steps - length - 1 > 0:
                after_visible[:, : time_steps - length - 1] = (
                    observation[:, 1 + length : time_steps]
                )
            random_score = random_score - 0.20 * after_visible.float()
            random_score = torch.where(valid, random_score, inf)
            scores[:, 1 : time_steps - length + 1] = random_score
            score_by_length.append(scores)

        stacked = torch.stack(score_by_length, dim=1)
        flat = stacked.reshape(batch, -1)
        minimum, flat_index = flat.min(dim=1)
        has_candidate = torch.isfinite(minimum)
        if not bool(has_candidate.any().item()):
            break

        per_length = time_steps * entities
        length_index = flat_index // per_length
        remainder = flat_index % per_length
        start = remainder // entities
        entity = remainder % entities
        ...
        for offset in range(maximum_length):
            active = has_candidate & (offset < chosen_length)
            if not bool(active.any().item()):
                continue
            rows = batch_index[active]
            times = start[active] + offset
            entities_selected = entity[active]
            selected[rows, times, entities_selected] = True

        stop = start + chosen_length
        valid_stop = has_candidate & (stop < time_steps)
        if bool(valid_stop.any().item()):
            rows = batch_index[valid_stop]
            times = stop[valid_stop]
            entities_selected = entity[valid_stop]
            visible_after = observation[rows, times, entities_selected]
            rows = rows[visible_after]
            times = times[visible_after]
            entities_selected = entities_selected[visible_after]
            reappearance[rows, times, entities_selected] = True

    return selected, reappearance


def inverse_dynamics_statistics(
    head: PerAgentInverseDynamicsHead | None,
    current_latent: torch.Tensor,
    predicted_next_latent: torch.Tensor,
    action_seq: torch.Tensor,
    action_mask_seq: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    zero = predicted_next_latent.float().sum() * 0.0
    if head is None:
        return zero, zero.detach()

    ally_count = int(action_seq.shape[2])
    current = current_latent[:, :, :ally_count]
    predicted = predicted_next_latent[:, :, :ally_count]
    logits = head(current, predicted)
    factual = action_seq[:, : current.shape[1], :ally_count]
    valid = (
        action_mask_seq[:, : current.shape[1], :ally_count].bool()
        & (factual.float().sum(dim=-1) > 0)
    )
    if not bool(valid.any().item()):
        return zero, zero.detach()
    target = factual.argmax(dim=-1)
    loss = F.cross_entropy(logits[valid].float(), target[valid])
    accuracy = (
        logits[valid].argmax(dim=-1) == target[valid]
    ).float().mean()
...
def markov_rollout_rnn_losses(
    ...
    inverse_dynamics_head: PerAgentInverseDynamicsHead | None,
    batch: dict[str, torch.Tensor],
    *,
    rollout_window: int,
    rollout_horizon: int,
    ...
    residual_state_decoder: bool,
    delta_loss_weight: float,
    event_dynamics_weight: float,
    event_dynamics_threshold: float,
    enemy_observation_dropout: float,
    hidden_reconstruction_weight: float,
    occlusion_mode: str,
    contiguous_occlusion_spans: list[int],
    occlusion_spans_per_sample: int,
    last_seen_anchor_weight: float,
    last_seen_change_threshold: float,
    hidden_presence_weight: float,
    reappearance_consistency_weight: float,
    ...
    if "observation_mask_seq" not in batch:
        raise RuntimeError(
            ...
            "visibility dataset before training."
        )
    original_observation_mask_seq = batch["observation_mask_seq"]

    if target_mode == "observed":
        target_entity_seq_full = entity_seq
        target_entity_mask_seq_full = original_observation_mask_seq
    elif target_mode == "full":
        target_entity_seq_full = batch.get("target_entity_seq", entity_seq)
        target_entity_mask_seq_full = batch.get(
            "target_entity_mask_seq", original_observation_mask_seq
        )
    else:
        raise ValueError(f"Unknown target_mode: {target_mode}")

    observation_mask_seq = original_observation_mask_seq
    artificial_hidden_seq = torch.zeros_like(
        original_observation_mask_seq,
        dtype=torch.bool,
    )
    artificial_reappearance_seq = torch.zeros_like(
        original_observation_mask_seq,
        dtype=torch.bool,
    )

    if occlusion_mode == "independent":
        dropout_probability = float(enemy_observation_dropout)
        if not 0.0 < dropout_probability < 1.0:
            raise ValueError(
                "independent occlusion requires enemy dropout in (0, 1)"
            )
        if "action_mask_seq" not in batch:
            raise RuntimeError(
                "enemy observation dropout requires action_mask_seq"
            )
        max_agents = int(batch["action_mask_seq"].shape[-1])
        enemy_slots = torch.zeros_like(
            original_observation_mask_seq,
            dtype=torch.bool,
        )
        enemy_slots[..., max_agents:] = True
        eligible = (
            original_observation_mask_seq.bool()
            & enemy_slots
            & batch["entity_slot_mask_seq"].bool()
            & batch["state_mask"].bool().unsqueeze(-1)
        )
        artificial_hidden_seq = (
            torch.rand_like(original_observation_mask_seq.float())
            < dropout_probability
        ) & eligible
        artificial_reappearance_seq[:, 1:] = (
            artificial_hidden_seq[:, :-1]
            & original_observation_mask_seq[:, 1:].bool()
        )
    elif occlusion_mode == "contiguous":
        max_agents = int(batch["action_mask_seq"].shape[-1])
        (
            artificial_hidden_seq,
            artificial_reappearance_seq,
        ) = build_contiguous_occlusion_schedule(
            original_observation_mask_seq,
            batch["entity_slot_mask_seq"],
            batch["state_mask"],
            max_agents=max_agents,
            spans=list(contiguous_occlusion_spans),
            spans_per_sample=int(occlusion_spans_per_sample),
        )
    elif occlusion_mode != "none":
        raise ValueError(f"Unknown occlusion mode: {occlusion_mode}")

    if bool(artificial_hidden_seq.any().item()):
        observation_mask_seq = (
            original_observation_mask_seq.bool()
            & ~artificial_hidden_seq
        ).to(original_observation_mask_seq.dtype)
        # Zero token values as a second leakage barrier. The encoder should
        # honor the observation mask, but the experiment must not rely on it.
        entity_seq = entity_seq.masked_fill(
            artificial_hidden_seq.unsqueeze(-1),
            0.0,
        )
    if target_mode == "observed":
        target_entity_seq_full = entity_seq
    ...
    for start_idx in range(p):
        ...
        action_fusion_module = getattr(
            memory_module, "action_fusion_module", None
        )
        if action_fusion_module is not None:
            pred_h_raw = pred_h_raw + action_fusion_module(
                action_h[:, 0],
                action_mask_h[:, 0],
                entities=entities,
                dtype=pred_h_raw.dtype,
            ) * future_slot_mask.unsqueeze(-1)
    ...
    for step in range(1, h):
        ...
        if action_fusion_module is not None:
            pred_raw_flat = pred_raw_flat + action_fusion_module(
                action_h_flat[:, 0],
                action_mask_h_flat[:, 0],
                entities=entities,
                dtype=pred_raw_flat.dtype,
            ) * future_slot_flat.unsqueeze(-1)
    ...
    # Event weighting affects dynamics consistency only; representation
    # regularizers remain on the original data distribution.
    reference_entity = (
        entity_seq[:, :p]
        .unsqueeze(2)
        .expand(-1, -1, h, -1, -1)
    ...
    changed_coordinate = (
        (target_entity - reference_entity).abs()
        > float(event_dynamics_threshold)
    ) & dynamic_reference_mask.bool()
    event_entity = changed_coordinate.any(dim=-1).to(latent_mask.dtype)
    event_latent_mask = latent_mask * (
        1.0 + float(event_dynamics_weight) * event_entity.unsqueeze(-1)
    )
    ...
    delta_loss = weighted_feature_mse(
        decoded - reference_entity,
        target_entity - reference_entity,
        decoded_dynamic_mask,
        aux_weights,
    )

    artificial_hidden_targets = gather_rollout_sequence(
        artificial_hidden_seq,
        p,
        h,
    )
    artificial_reappearance_targets = gather_rollout_sequence(
        artificial_reappearance_seq,
        p,
        h,
    )
    hidden_reconstruction_mask = (
        base_dynamic_valid
        * artificial_hidden_targets.unsqueeze(-1)
        * target_valid
    )
    hidden_reconstruction_loss = weighted_feature_mse(
        decoded,
        target_entity,
        hidden_reconstruction_mask,
        aux_weights,
    ...
    (
        last_seen_before_seq,
        has_last_seen_before_seq,
        last_seen_age_seq,
    ) = last_seen_before_sequence(
        target_entity_seq_full,
        observation_mask_seq,
    )
    last_seen_target = gather_rollout_sequence(
        last_seen_before_seq,
        p,
        h,
    )
    has_last_seen_target = gather_rollout_sequence(
        has_last_seen_before_seq,
        p,
        h,
    )
    last_seen_age_target = gather_rollout_sequence(
        last_seen_age_seq,
        p,
        h,
    )
    hidden_with_history_mask = (
        hidden_reconstruction_mask
        * has_last_seen_target.unsqueeze(-1)
    )
    unchanged_since_last_seen = (
        (target_entity - last_seen_target).abs()
        <= float(last_seen_change_threshold)
    ).to(decoded.dtype)
    last_seen_anchor_mask = (
        hidden_with_history_mask * unchanged_since_last_seen
    )
    last_seen_anchor_loss = weighted_feature_mse(
        decoded,
        last_seen_target,
        last_seen_anchor_mask,
        aux_weights,
    ...
    reappearance_mask = (
        base_dynamic_valid
        * artificial_reappearance_targets.unsqueeze(-1)
        * target_valid
    )
    reappearance_consistency_loss = weighted_feature_mse(
        decoded,
        target_entity,
        reappearance_mask,
        aux_weights,
    ...
    hidden_presence_mask = (
        artificial_hidden_targets.to(presence_mask.dtype)
        * presence_mask
    )
    hidden_presence_loss = weighted_bce(
        presence_logits,
        target_entity_mask,
        hidden_presence_mask,
        aux_weights,
    ...
    inverse_dynamics_loss, inverse_dynamics_accuracy = (
        inverse_dynamics_statistics(
    ...
    total_loss = (
        ...
        * memory_barlow_stats["normalized"]
        ...
        + float(hidden_reconstruction_weight)
        * hidden_reconstruction_loss
        + float(last_seen_anchor_weight) * last_seen_anchor_loss
        + float(hidden_presence_weight) * hidden_presence_loss
        + float(reappearance_consistency_weight)
        * reappearance_consistency_loss
        + float(inverse_dynamics_weight) * inverse_dynamics_loss
    ...
    losses: dict[str, torch.Tensor] = {
        ...
        "delta_state_loss": delta_loss,
        "weighted_delta_state_loss": (
            float(delta_loss_weight) * delta_loss
        ).detach(),
        "hidden_reconstruction_loss": hidden_reconstruction_loss,
        "weighted_hidden_reconstruction_loss": (
            float(hidden_reconstruction_weight)
            * hidden_reconstruction_loss
        ).detach(),
        "artificial_hidden_fraction": (
            (
                artificial_hidden_seq
                & original_observation_mask_seq.bool()
            ).sum()
            / original_observation_mask_seq.sum().clamp_min(1.0)
        ).detach(),
        "scheduled_hidden_fraction": (
            artificial_hidden_seq.float().mean()
        ).detach(),
        "last_seen_anchor_loss": last_seen_anchor_loss,
        "weighted_last_seen_anchor_loss": (
            float(last_seen_anchor_weight) * last_seen_anchor_loss
        ).detach(),
        "hidden_presence_loss": hidden_presence_loss,
        "weighted_hidden_presence_loss": (
            float(hidden_presence_weight) * hidden_presence_loss
        ).detach(),
        "reappearance_consistency_loss": (
            reappearance_consistency_loss
        ),
        "weighted_reappearance_consistency_loss": (
            float(reappearance_consistency_weight)
            * reappearance_consistency_loss
        ).detach(),
        "inverse_dynamics_loss": inverse_dynamics_loss,
        "weighted_inverse_dynamics_loss": (
            float(inverse_dynamics_weight) * inverse_dynamics_loss
        ).detach(),
        "inverse_dynamics_accuracy": inverse_dynamics_accuracy,
        "hidden_last_seen_age_mean": torch.where(
            artificial_hidden_targets.bool()
            & has_last_seen_target.bool(),
            last_seen_age_target.float(),
            torch.zeros_like(last_seen_age_target.float()),
        ).sum().detach()
        / (
            artificial_hidden_targets.bool()
            & has_last_seen_target.bool()
        ).sum().clamp_min(1).detach(),
        "event_entity_fraction": event_entity.mean().detach(),
...
def validate_resume_configuration(
    ...
    """Reject legacy or cross-experiment checkpoint resumes."""
    if int(checkpoint_config.get("explicit_visibility_mask_version", 0)) < 1:
        raise SystemExit(
            "Resume rejected: checkpoint predates the explicit visibility "
            "mask fix. Start this experiment from scratch."
        )

    expected: dict[str, object] = {
        "ema_target_encoder": bool(args.ema_target_encoder),
        "ema_momentum": float(args.ema_momentum),
        "memory_barlow_scale": float(args.memory_barlow_scale),
        "event_balanced_sampling": bool(args.event_balanced_sampling),
        "event_fraction": float(args.event_fraction),
        "event_pool_fraction": float(args.event_pool_fraction),
        "rollout_window": int(args.rollout_window),
        "rollout_horizon": int(args.rollout_horizon),
        "target_mode": str(args.target_mode),
        "action_conditioned_memory": bool(args.action_conditioned_memory),
        "enemy_visibility_mask": bool(args.enemy_visibility_mask),
        "enemy_sight_range": float(args.enemy_sight_range),
        "one_step_weight": float(args.one_step_weight),
        "r2_dyn_scale": float(args.r2_dyn_scale),
        "r2_rep_scale": float(args.r2_rep_scale),
        "r2_barlow_scale": float(args.r2_barlow_scale),
        "sigreg_weight": float(args.sigreg_weight),
        "decoder_weight": float(args.decoder_weight),
        ...
        "residual_state_decoder": bool(args.residual_state_decoder),
        "delta_loss_weight": float(args.delta_loss_weight),
        "event_dynamics_weight": float(args.event_dynamics_weight),
        "event_dynamics_threshold": float(
            args.event_dynamics_threshold
        ),
        "direct_action_fusion": bool(args.direct_action_fusion),
        "direct_action_hidden_dim": int(args.direct_action_hidden_dim),
        "enemy_observation_dropout": float(
            args.enemy_observation_dropout
        ),
        "hidden_reconstruction_weight": float(
            args.hidden_reconstruction_weight
        ),
        "occlusion_mode": str(args.occlusion_mode),
        "contiguous_occlusion_spans": [
            int(value) for value in args.contiguous_occlusion_spans
        ],
        "occlusion_spans_per_sample": int(
            args.occlusion_spans_per_sample
        ),
        "last_seen_anchor_weight": float(
            args.last_seen_anchor_weight
        ),
        "last_seen_change_threshold": float(
            args.last_seen_change_threshold
        ),
        "hidden_presence_weight": float(args.hidden_presence_weight),
        "reappearance_consistency_weight": float(
            args.reappearance_consistency_weight
        ),
        "inverse_dynamics_weight": float(
            args.inverse_dynamics_weight
        ),
        "inverse_dynamics_hidden_dim": int(
            args.inverse_dynamics_hidden_dim
        ),
    }
    mismatch: list[str] = []
    for key, expected_value in expected.items():
        ...
        if not matches:
            mismatch.append(
                f"{key}: checkpoint={actual!r}, requested={expected_value!r}"
            )
    if mismatch:
        raise SystemExit(
            "Resume rejected because the checkpoint does not match this "
            "experiment:\n  " + "\n  ".join(mismatch)
        )
...
def validate_dataset_visibility(
    ...
    """Catch the old zero-hidden-mask bug before any expensive training."""
    if not dataset.enemy_visibility_mask:
        return
    if int(getattr(dataset, "explicit_visibility_mask_version", 0)) < 1:
        raise RuntimeError("Corrected visibility dataset was not installed.")

    segment_count = len(dataset.index)
    item_count = min(segment_count, int(maximum_items))
    if item_count <= 0:
        raise RuntimeError("Visibility preflight received an empty dataset.")
    ...
    for index in indices:
        episode_index, segment_start = dataset.index[int(index)]
    ...
    if hidden <= 0.0:
        raise RuntimeError(
            ...
            f"{item_count} sampled segments. Refusing to train."
        )
    print(
        "visibility_preflight_passed "
        f"items={item_count} hidden_fraction={hidden / max(target, 1.0):.6f}",
        flush=True,
    )
...
def main() -> None:
    ...
    artificial_objective_enabled = any(
        float(value) > 0.0
        for value in (
            args.hidden_reconstruction_weight,
            args.last_seen_anchor_weight,
            args.hidden_presence_weight,
            args.reappearance_consistency_weight,
        )
    )
    if artificial_objective_enabled and args.occlusion_mode == "none":
        raise SystemExit(
            "Artificial hidden objectives require --occlusion-mode "
            "independent or contiguous"
        )
    if (
        args.occlusion_mode == "none"
        and args.enemy_observation_dropout != 0.0
    ):
        raise SystemExit(
            "--enemy-observation-dropout is only used with "
            "--occlusion-mode independent"
        )
    if args.occlusion_mode == "independent" and not (
        0.0 < args.enemy_observation_dropout < 1.0
    ):
        raise SystemExit(
            "independent occlusion requires "
            "--enemy-observation-dropout in (0, 1)"
        )
    if args.occlusion_mode == "contiguous" and (
        args.enemy_observation_dropout != 0.0
    ):
        raise SystemExit(
            "contiguous occlusion does not use "
            "--enemy-observation-dropout; set it to 0"
        )
    if any(
        value > 0.0
        for value in (
    ...
    ) and args.occlusion_mode != "contiguous":
        raise SystemExit(
            "last-seen, hidden-presence, and reappearance objectives "
            "require --occlusion-mode contiguous"
        )
    if args.event_dynamics_weight < 0.0:
        raise SystemExit("--event-dynamics-weight must be >= 0")
    if args.delta_loss_weight < 0.0:
        raise SystemExit("--delta-loss-weight must be >= 0")
    if args.hidden_reconstruction_weight < 0.0:
        raise SystemExit("--hidden-reconstruction-weight must be >= 0")
    if not 0.0 <= args.enemy_observation_dropout < 1.0:
        raise SystemExit(
            "--enemy-observation-dropout must be in [0, 1)"
        )
    if args.direct_action_hidden_dim < 1:
        raise SystemExit("--direct-action-hidden-dim must be >= 1")
    if args.inverse_dynamics_hidden_dim < 1:
        raise SystemExit("--inverse-dynamics-hidden-dim must be >= 1")
    if args.occlusion_spans_per_sample < 1:
        raise SystemExit("--occlusion-spans-per-sample must be >= 1")
    if any(value < 1 for value in args.contiguous_occlusion_spans):
        raise SystemExit(
            "--contiguous-occlusion-spans must all be >= 1"
        )
    for name, value in (
        ("last-seen-anchor-weight", args.last_seen_anchor_weight),
        ("hidden-presence-weight", args.hidden_presence_weight),
        (
            "reappearance-consistency-weight",
            args.reappearance_consistency_weight,
        ),
        ("inverse-dynamics-weight", args.inverse_dynamics_weight),
    ):
        if value < 0.0:
            raise SystemExit(f"--{name} must be >= 0")
    if args.last_seen_change_threshold < 0.0:
        raise SystemExit(
            "--last-seen-change-threshold must be >= 0"
        )
    if args.event_dynamics_threshold < 0.0:
        raise SystemExit("--event-dynamics-threshold must be >= 0")
    ...
    initial_autocast_enabled = runtime_autocast_enabled
    ...
    print(
        ...
        f"autocast={runtime_autocast_enabled} "
        f"grad_scaler={scaler_enabled} "
        f"amp_init_scale={args.amp_init_scale} "
        f"amp_growth_interval={args.amp_growth_interval}",
        flush=True,
    )
    ...
    action_fusion_module = None
    if args.direct_action_fusion:
        action_fusion_module = DirectOrderedActionFusion(
            n_actions=dataset.metadata.n_actions,
            max_agents=dataset.metadata.max_agents,
            max_entities=(
                dataset.metadata.max_agents
                + dataset.metadata.max_enemies
            ),
            latent_dim=int(arch["latent_dim"]),
            hidden_dim=args.direct_action_hidden_dim,
        ).to(device)
        # Keep this as a non-registered runtime attachment. It has its own
        # optimizer entries and checkpoint state. Registering it as a child of
        # memory_module would duplicate optimizer parameters and checkpoint keys.
        object.__setattr__(
            memory_module,
            "action_fusion_module",
            action_fusion_module,
        )

    inverse_dynamics_head = None
    if args.inverse_dynamics_weight > 0.0:
        inverse_dynamics_head = PerAgentInverseDynamicsHead(
            latent_dim=int(arch["latent_dim"]),
            n_actions=dataset.metadata.max_actions,
            hidden_dim=args.inverse_dynamics_hidden_dim,
        ).to(device)
    ...
    named_trainable_parameters = (
        ...
        + [
            (f"memory_module.{name}", parameter)
            for name, parameter in memory_module.named_parameters()
        ]
        + [
            (f"r2_projector.{name}", parameter)
            for name, parameter in r2_projector.named_parameters()
        ]
        + [
            (f"memory_projector.{name}", parameter)
            for name, parameter in memory_projector.named_parameters()
        ]
        + (
            [
                (f"action_fusion_module.{name}", parameter)
                for name, parameter in action_fusion_module.named_parameters()
            ]
            if action_fusion_module is not None
            else []
        )
        + (
            [
                (f"inverse_dynamics_head.{name}", parameter)
                for name, parameter in inverse_dynamics_head.named_parameters()
            ]
            if inverse_dynamics_head is not None
            else []
        )
    )
    filtered_named_parameters: list[
    ...
    for name, parameter in named_trainable_parameters:
        ...
        filtered_named_parameters.append((name, parameter))

    if duplicate_parameter_names:
        raise RuntimeError(
            "Duplicate optimizer parameters detected before training: "
            + ", ".join(duplicate_parameter_names)
        )

    named_trainable_parameters = filtered_named_parameters
    trainable_parameters = [
        parameter for _, parameter in named_trainable_parameters
    ]
    print(
        "optimizer_parameter_check "
        f"unique_tensors={len(trainable_parameters)} "
        f"unique_scalars={sum(p.numel() for p in trainable_parameters)} "
        "duplicates=0",
        flush=True,
    )
    ...
    scaler = torch.cuda.amp.GradScaler(
        ...
        growth_factor=2.0,
        backoff_factor=0.5,
        growth_interval=max(int(args.amp_growth_interval), 1),
    )

    start_epoch = 1
    global_step = 0
    successful_optimizer_steps = 0
    total_nonfinite_grad_steps = 0
    ...
    if args.init_from:
        ...
        memory_module.load_state_dict(
            init_checkpoint["memory_module_state"]
        )
        if "r2_projector_state" in init_checkpoint:
            r2_projector.load_state_dict(
                init_checkpoint["r2_projector_state"]
            )
        if "memory_projector_state" in init_checkpoint:
            memory_projector.load_state_dict(
                init_checkpoint["memory_projector_state"]
            )
        if target_encoder is not None:
            target_state = init_checkpoint.get("target_encoder_state")
            if target_state is not None:
                target_encoder.load_state_dict(target_state)
            else:
                target_encoder.load_state_dict(model.encoder.state_dict())
        if inverse_dynamics_head is not None:
            inverse_state = init_checkpoint.get(
                "inverse_dynamics_head_state"
            )
            if inverse_state is not None:
                inverse_dynamics_head.load_state_dict(inverse_state)
            else:
                print(
                    "init_from: inverse dynamics head starts fresh",
                    flush=True,
                )
        print(
            f"initialized_weights_from {args.init_from}",
            flush=True,
        )
    ...
    if args.resume:
        ...
        if int(checkpoint_config.get("r2_blocker_fix_version", 0)) < 1:
            raise SystemExit(
                ...
                "checkpoint. Start a new run directory instead."
            )
        validate_resume_configuration(checkpoint_config, args)
        ...
        if action_fusion_module is not None:
            action_fusion_state = checkpoint.get(
                "action_fusion_module_state"
            )
            if action_fusion_state is None:
                raise SystemExit(
                    "Cannot resume direct-action-fusion run without "
                    "action_fusion_module_state"
                )
            action_fusion_module.load_state_dict(action_fusion_state)
        if inverse_dynamics_head is not None:
            inverse_state = checkpoint.get("inverse_dynamics_head_state")
            if inverse_state is None:
                raise SystemExit(
                    "Cannot resume inverse-dynamics run without "
                    "inverse_dynamics_head_state"
                )
            inverse_dynamics_head.load_state_dict(inverse_state)
        ...
        if "scaler_state" in checkpoint and scaler.is_enabled():
            try:
                ...
                print(
                    "warning: could not restore GradScaler state; "
                    f"using fresh stable scaler ({type(exc).__name__}: {exc})",
                    flush=True,
                )
        checkpoint_epoch = int(checkpoint.get("epoch", 0))
        checkpoint_epoch_complete = bool(
            checkpoint.get("epoch_complete", True)
        )
        if checkpoint_epoch_complete:
            ...
            print(
                "resuming_mid_epoch "
                f"epoch={start_epoch} "
                f"batches_completed={resume_batches_completed}",
                flush=True,
            )
        global_step = int(checkpoint.get("global_step", 0))
        successful_optimizer_steps = int(
            checkpoint.get("successful_optimizer_steps", global_step)
        )
        total_nonfinite_grad_steps = int(
            checkpoint.get("total_nonfinite_grad_steps", 0)
        )

    saved_config = vars(args) | arch | {
        "resolved_device": device.type,
        "amp_enabled_initial": initial_autocast_enabled,
        ...
        "r2_blocker_fix_version": 4,
        "experiment_suite": "exp31_exp33_belief_planning",
        "explicit_visibility_mask_version": 1,
        "observation_mask_source": "dataset_observation_mask_seq",
        ...
        "stability_hardening_version": 1,
        "amp_dtype_requested": args.amp_dtype,
        "precision_policy_initial": precision_policy,
        "amp_init_scale": args.amp_init_scale,
        "amp_growth_interval": args.amp_growth_interval,
        "amp_fallback_after_nonfinite": (
            args.amp_fallback_after_nonfinite
        ),
        "aux_loss_warmup_steps": args.aux_loss_warmup_steps,
        "adam_eps": args.adam_eps,
        "nonfinite_lr_backoff": args.nonfinite_lr_backoff,
        "event_balanced_sampling": args.event_balanced_sampling,
        "event_sampling_stats": event_sampling_stats,
        "structural_suite_version": 1,
        "residual_state_decoder": args.residual_state_decoder,
        "delta_loss_weight": args.delta_loss_weight,
        "event_dynamics_weight": args.event_dynamics_weight,
        "event_dynamics_threshold": args.event_dynamics_threshold,
        "direct_action_fusion": args.direct_action_fusion,
        "direct_action_hidden_dim": args.direct_action_hidden_dim,
        "enemy_observation_dropout": args.enemy_observation_dropout,
        ...
        "occlusion_spans_per_sample": args.occlusion_spans_per_sample,
        "last_seen_anchor_weight": args.last_seen_anchor_weight,
        "last_seen_change_threshold": args.last_seen_change_threshold,
        "hidden_presence_weight": args.hidden_presence_weight,
        "reappearance_consistency_weight": (
            args.reappearance_consistency_weight
        ),
    ...
    if args.wandb:
        ...
        if action_fusion_module is not None:
            wandb_run.watch(action_fusion_module, log=None)
        if inverse_dynamics_head is not None:
            wandb_run.watch(inverse_dynamics_head, log=None)
        ...
        torch.save(
            {
                ...
                "action_fusion_module_state": (
                    action_fusion_module.state_dict()
                    if action_fusion_module is not None
                    else None
                ),
                "inverse_dynamics_head_state": (
                    inverse_dynamics_head.state_dict()
                    if inverse_dynamics_head is not None
                    else None
                ),
                ...
                "total_nonfinite_grad_steps": total_nonfinite_grad_steps,
            },
            checkpoint_path,
        )
    ...
    if action_fusion_module is not None:
        action_fusion_module.train()
    if inverse_dynamics_head is not None:
        inverse_dynamics_head.train()
    if target_encoder is not None:
        target_encoder.eval()
    r2_projector.train()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_index, batch in enumerate(loader, start=1):
            ...
            bad_batch_fields = nonfinite_batch_fields(batch)
            if bad_batch_fields:
                total_nonfinite_grad_steps += 1
                consecutive_nonfinite_grad_steps += 1
                print(
                    "SKIP_NONFINITE_INPUT "
                    f"epoch={epoch} step={global_step} "
                    f"fields={bad_batch_fields}",
                    flush=True,
                )
                continue
            ...
            if args.aux_loss_warmup_steps > 0:
                auxiliary_warmup = min(
                    1.0,
                    float(successful_optimizer_steps + 1)
                    / float(args.aux_loss_warmup_steps),
                )
            else:
                auxiliary_warmup = 1.0

            effective_sigreg_weight = (
                float(active_sigreg_weight) * auxiliary_warmup
            )
            effective_barlow_scale = (
                float(args.r2_barlow_scale) * auxiliary_warmup
            )
            effective_memory_barlow_scale = (
                float(args.memory_barlow_scale) * auxiliary_warmup
            )

            autocast_context = (
                torch.autocast(
                    device_type="cuda",
                    dtype=runtime_autocast_dtype,
                    enabled=runtime_autocast_enabled,
                )
                if device.type == "cuda"
                else nullcontext()
            )
            ...
            with autocast_context:
                losses = markov_rollout_rnn_losses(
                    ...
                    r2_barlow_scale=effective_barlow_scale,
                    memory_barlow_scale=effective_memory_barlow_scale,
                    ...
                    delta_loss_weight=args.delta_loss_weight,
                    event_dynamics_weight=args.event_dynamics_weight,
                    event_dynamics_threshold=(
                        args.event_dynamics_threshold
                    ),
                    enemy_observation_dropout=(
                        args.enemy_observation_dropout
                    ),
                    hidden_reconstruction_weight=(
                        args.hidden_reconstruction_weight
                    ...
                    occlusion_mode=args.occlusion_mode,
                    contiguous_occlusion_spans=(
                        args.contiguous_occlusion_spans
                    ),
                    occlusion_spans_per_sample=(
                        args.occlusion_spans_per_sample
                    ),
                    last_seen_anchor_weight=(
                        args.last_seen_anchor_weight
                    ),
                    last_seen_change_threshold=(
                        args.last_seen_change_threshold
                    ),
                    hidden_presence_weight=(
                        args.hidden_presence_weight
                    ),
                    reappearance_consistency_weight=(
                        args.reappearance_consistency_weight
                    ),
                    inverse_dynamics_weight=(
                        args.inverse_dynamics_weight
                    ),
                )
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                trainable_parameters,
                args.grad_clip if args.grad_clip > 0 else 1.0e30,
                error_if_nonfinite=False,
            )
            grad_norm_is_finite = bool(
            ...
            if not grad_norm_is_finite:
                total_nonfinite_grad_steps += 1
                consecutive_nonfinite_grad_steps += 1
                bad_gradient_names = nonfinite_gradient_names(
                    named_trainable_parameters
                )
                ...
                if scaler.is_enabled():
                    reduced_scale = max(previous_scale * 0.5, 1.0)
                    scaler.update(new_scale=reduced_scale)
                else:
                    reduced_scale = previous_scale

                fallback_triggered = (
                    args.amp_fallback_after_nonfinite > 0
                    and consecutive_nonfinite_grad_steps
                    >= args.amp_fallback_after_nonfinite
                )

                if fallback_triggered and runtime_autocast_enabled:
                    runtime_autocast_enabled = False
                    runtime_autocast_dtype = None
                    scaler = torch.cuda.amp.GradScaler(enabled=False)
                    old_lr, new_lr = apply_learning_rate_backoff(
                        optimizer,
                        scheduler,
                        factor=args.nonfinite_lr_backoff,
                        minimum=args.min_learning_rate,
                    )
                    print(
                        "PRECISION_FALLBACK_TO_FP32 "
                        f"epoch={epoch} step={global_step} "
                        f"lr={old_lr:.8g}->{new_lr:.8g}",
                        flush=True,
                    )
                    consecutive_nonfinite_grad_steps = 0
                elif (
                    fallback_triggered
                    and not runtime_autocast_enabled
                ):
                    old_lr, new_lr = apply_learning_rate_backoff(
                        optimizer,
                        scheduler,
                ...
                recovery_path = out_dir / "checkpoint_recovery.pt"
                save_checkpoint(
                    epoch,
                    recovery_path,
                    epoch_complete=False,
                    batches_completed_in_epoch=epoch_batches,
                )

                message = (
                    "SKIP_NONFINITE_GRADIENT "
                    f"epoch={epoch} step={global_step} "
                    f"consecutive={consecutive_nonfinite_grad_steps} "
                    f"total={total_nonfinite_grad_steps} "
                    f"amp_scale={previous_scale:.1f}->{reduced_scale:.1f} "
                    f"bad_parameters={bad_gradient_names}"
                )
                ...
                if wandb_run is not None:
                    wandb_run.log(
                        {
                            "train/nonfinite_gradient_step": 1,
                            "train/nonfinite_gradient_steps_total": (
                                total_nonfinite_grad_steps
                            ),
                            "train/amp_scale_after_skip": reduced_scale,
                        },
                        step=global_step,
                    )

                too_many_consecutive = (
                    args.max_consecutive_nonfinite_grad_steps > 0
                    and consecutive_nonfinite_grad_steps
                    >= args.max_consecutive_nonfinite_grad_steps
                )
                too_many_total = (
                    args.max_total_nonfinite_grad_steps > 0
                    and total_nonfinite_grad_steps
                    >= args.max_total_nonfinite_grad_steps
                )
                if too_many_consecutive or too_many_total:
                    raise RuntimeError(
                        "Exceeded non-finite-gradient safety limit. "
                        f"Recovery checkpoint: {recovery_path}"
                    )
                continue
            ...
            if (
                ...
                and successful_optimizer_steps
                % args.checkpoint_every_steps
                == 0
            ):
                save_checkpoint(
                    epoch,
                    out_dir / "checkpoint_recovery.pt",
                    epoch_complete=False,
                    batches_completed_in_epoch=epoch_batches,
                )
                print(
                    "saved_recovery_checkpoint "
                    f"epoch={epoch} step={global_step} "
                    f"successful_updates={successful_optimizer_steps}",
                    flush=True,
                )
            ...
            row: dict[str, float | int] = {
                ...
                "effective_barlow_scale": effective_barlow_scale,
                "effective_memory_barlow_scale": (
                    effective_memory_barlow_scale
                ),
                "auxiliary_warmup": auxiliary_warmup,
                "runtime_autocast_enabled": int(
                    runtime_autocast_enabled
                ),
                "runtime_precision_bf16": int(
                    runtime_autocast_dtype == torch.bfloat16
                ),
                "runtime_precision_fp16": int(
                    runtime_autocast_dtype == torch.float16
                ),
                "grad_norm": float(grad_norm.detach().cpu()),
                "nonfinite_gradient_step": 0,
        ...
        if epoch_logged_batches == 0:
            raise RuntimeError(
                f"Epoch {epoch} completed without a successful optimizer "
                "update; refusing to save a completed checkpoint."
            )
        ...
        for key, value in epoch_sums.items():
            epoch_row[key] = value / max(epoch_logged_batches, 1)
        ...
        save_checkpoint(
            ...
            epoch_checkpoint_path,
            epoch_complete=True,
            batches_completed_in_epoch=len(loader),
        )
        save_checkpoint(
            epoch,
        ...
        print(
            f"saved_checkpoint {epoch_checkpoint_path} "
            f"and {out_dir / 'checkpoint.pt'}",
            flush=True,
        )
        resume_batches_completed = 0
