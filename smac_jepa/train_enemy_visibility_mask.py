...
from smac_jepa.data.dataset_visibility_mask import SMACJEPADataset
from smac_jepa.data import load_manifest, load_manifest_all
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Mask enemy entity tokens in the input when they are not visible "
            "from any alive ally under the SMACLite sight-range rule. Targets "
            "remain full-state, so the model is still trained to predict hidden enemies."
        ),
    )
    parser.add_argument(
        "--enemy-sight-range",
