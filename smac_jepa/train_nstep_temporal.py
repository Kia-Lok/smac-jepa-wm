...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train entity-token SMAC-JEPA with configurable n-step temporal prediction loss and optional category-prioritized sampling"
    )
    ...
    parser.add_argument(
        ...
        default="uniform",
        help=(
            "Temporal prediction loss mode over the predicted n-step window. "
            "uniform gives every horizon equal weight; lambda uses td_lambda**h; "
            "flat-decay keeps early horizons at 1 then linearly decays. Default: uniform"
        ),
    )
    parser.add_argument(
        "--loss-horizon",
        ...
        default=None,
        help=(
            "Optional number of future prediction steps to include in the temporal loss. "
            "If omitted, uses the full predicted window. Example: --context-len 8 --window-len 8 --loss-horizon 4 "
            "trains on only the first 4 predicted steps."
        ),
    )
    parser.add_argument(
        "--flat-decay-start",
        ...
        help=(
            ...
            "Defaults to half of the active loss horizon."
        ),
    )
    parser.add_argument(
...
def _temporal_time_weights(
    ...
    flat_decay_start: int | None = None,
    flat_decay_final_weight: float = 0.5,
) -> torch.Tensor:
    """Return [length] horizon weights for n-step temporal prediction.

    This is not value-function TD. It is supervised multi-step world-model
    prediction loss over the predicted window.
    """
    if length <= 0:
        raise ValueError(f"temporal loss length must be positive, got {length}")

    if loss_horizon is None:
        active = length
    else:
        if loss_horizon <= 0:
            raise ValueError(f"--loss-horizon must be positive when provided, got {loss_horizon}")
        active = min(int(loss_horizon), length)
    ...
    if mode == "uniform":
        ...
        steps = torch.arange(active, device=device, dtype=dtype)
        weights[:active] = torch.pow(torch.as_tensor(td_lambda, device=device, dtype=dtype), steps)
    elif mode == "flat-decay":
        if not 0.0 <= flat_decay_final_weight <= 1.0:
            raise ValueError(
                f"--flat-decay-final-weight must be in [0, 1], got {flat_decay_final_weight}"
            )
        if flat_decay_start is None:
            flat = max(1, active // 2)
        else:
            if flat_decay_start <= 0:
                raise ValueError(f"--flat-decay-start must be positive, got {flat_decay_start}")
            flat = min(int(flat_decay_start), active)
        weights[:flat] = 1.0
        if active > flat:
            # Linearly decay from 1.0 after the flat region to final weight at the final active horizon.
            tail = torch.linspace(
                1.0,
            ...
            weights[flat:active] = tail
    else:
        raise ValueError(f"Unknown --temporal-loss mode: {mode!r}")

    if float(weights.sum().detach().cpu()) <= 0.0:
        raise ValueError(
            f"Temporal weights sum to zero: mode={mode!r}, length={length}, loss_horizon={loss_horizon}"
        )
    return weights
...
def temporal_prediction_mse(
    ...
    """Compute configurable n-step temporal prediction MSE.

    Returns:
        temporal_loss: scalar weighted mean over batch/window/features
        uniform_loss: scalar ordinary masked mean over the full prediction window
        temporal_weight_sum: scalar sum of active horizon weights
        per_sample_temporal_loss: [B] loss for category tracking
        weights: [T] horizon weights used
    """
    ...
    if pred.ndim < 2:
        ...
        weights = torch.ones(1, device=pred.device, dtype=pred.dtype)
        return loss, loss, one, per_sample.detach(), weights

    window_steps = pred.shape[1]
    weights = _temporal_time_weights(
        window_steps,
        mode=mode,
        td_lambda=td_lambda,
        device=pred.device,
    ...
    per_sample_temporal_loss = weighted_num_per_sample / weighted_den_per_sample
    temporal_loss = per_sample_temporal_loss.mean()

    uniform_num = (squared * uniform_mask).sum()
    uniform_den = uniform_mask.sum().clamp_min(1.0)
    uniform_loss = uniform_num / uniform_den

    return temporal_loss, uniform_loss, weights.sum(), per_sample_temporal_loss.detach(), weights.detach()


def base_losses_from_outputs(
    model: SMACJEPA,
    out: dict[str, torch.Tensor],
    sigreg_weight: float,
) -> dict[str, torch.Tensor]:
    """Exact one-forward equivalent of SMACJEPA.loss(...), using model.forward output."""
    ...
    total = (
        ...
        + getattr(model, "decoder_weight", 1.0) * decoded_loss
        + presence_loss
    )
...
def temporal_jepa_losses(
    ...
    """Compute configurable n-step temporal prediction objective with one forward pass."""
    ...
    base_losses = base_losses_from_outputs(model, out, sigreg_weight=sigreg_weight)

    temporal_pred_loss, uniform_pred_loss, temporal_weight_sum, pred_loss_per_sample, weights = temporal_prediction_mse(
        out["pred_latent"],
        out["target_latent"],
        out.get("mask"),
    ...
    total_loss = temporal_pred_loss + extra_losses

    losses = dict(base_losses)
    losses["total_loss"] = total_loss
    losses["pred_loss"] = temporal_pred_loss
    losses["pred_loss_uniform"] = uniform_pred_loss.detach()
    losses["temporal_weight_sum"] = temporal_weight_sum.detach()
    losses["lambda_weight_sum"] = temporal_weight_sum.detach()  # Backward-compatible log column name.
    losses["active_loss_horizon"] = torch.as_tensor(
        int((weights > 0).sum().detach().cpu()),
        ...
        for batch in active_loader:
            ...
            if category_enabled:
                ...
                if "pred_loss_per_sample" not in losses:
                    raise RuntimeError(
                        "Category-prioritized training needs 'pred_loss_per_sample' from temporal_jepa_losses."
                    )
