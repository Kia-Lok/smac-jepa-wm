...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "entities. Requires --enemy-observation-dropout > 0."
        ),
    )
...
def markov_rollout_rnn_losses(
    ...
    *,
    ...
    artificial_hidden_seq = torch.zeros_like(original_observation_mask_seq)

    dropout_probability = float(enemy_observation_dropout)
    if dropout_probability > 0.0:
        if not 0.0 <= dropout_probability < 1.0:
            raise ValueError("--enemy-observation-dropout must be in [0, 1)")
        ...
        artificial_hidden_seq = random_hide.to(
            original_observation_mask_seq.dtype
        )
        observation_mask_seq = (
            original_observation_mask_seq
            * (~random_hide).to(original_observation_mask_seq.dtype)
        )
    ...
    artificial_hidden_targets = torch.stack(
        [
            torch.stack(
                [
                    artificial_hidden_seq[:, start + step + 1]
                    for step in range(h)
                ],
                dim=1,
            )
...
def main() -> None:
    ...
    if args.hidden_reconstruction_weight > 0.0 and (
        args.enemy_observation_dropout <= 0.0
    ):
        raise SystemExit(
            "--hidden-reconstruction-weight requires "
            "--enemy-observation-dropout > 0"
        )
    ...
    saved_config = vars(args) | arch | {
        ...
        "experiment_suite": "exp17_exp19_explicit_visibility_ablations",
