...
"""
Train SMAC-JEPA with Markov recursive rollout + RNN memory + enemy visibility masking.
...
def markov_rollout_rnn_losses(
    ...
    *,
    ...
    """
    Compute recursive Markov rollout losses with RNN memory.

    The current rollout latent is always the previous predicted latent z_hat,
    except step 0, where it starts from true encode(s_t).

    Memory is attached at every step:
        z_conditioned = memory_module.condition(z_current, memory_current)
        z_next = predictor(z_conditioned, action)
        memory_next = memory_module.update(z_next, memory_current)
    """
    entity_seq = batch["entity_seq"]                       # visibility-masked inputs
    entity_mask_seq = batch["entity_mask_seq"]             # visibility-masked input masks
    target_entity_seq_full = batch.get("target_entity_seq", entity_seq)              # full-state targets
    ...
    input_latents = model.encoder(entity_seq, entity_mask_seq)  # observed/visibility-masked latents
    target_latents = model.encoder(target_entity_seq_full, target_entity_mask_seq_full)  # full-state target latents
    _, _, entities, latent_dim = input_latents.shape

    # Initial state for each rollout start comes from the observed/visibility-masked input.
    z = input_latents[:, :p].reshape(bsz * p, entities, latent_dim)
    current_entity_mask = entity_mask_seq[:, :p].reshape(bsz * p, entities)

    # Initial memory is null/zero for every rollout start and entity slot.
    ...
    for step in range(h):
        ...
        target_mask_h = target_entity_mask_seq_full[:, step + 1 : step + p + 1]
        entity_mask_h = current_entity_mask.unsqueeze(1)
        entity_mask_current = current_entity_mask

        # Attach memory to every rollout step.
        z_conditioned = memory_module.condition(z, memory, entity_mask_current)
        ...
        valid_entity_h = entity_mask_current.unsqueeze(-1)
        pred_h = pred_h * valid_entity_h

        # Update memory using the latest predicted latent z_hat.
        memory = memory_module.update(pred_h, memory, target_mask_h.reshape(bsz * p, entities))
        current_entity_mask = target_mask_h.reshape(bsz * p, entities)
        memory_norms.append(memory.detach().float().norm(dim=-1).mean())

        target_h = target_latents[:, step + 1 : step + p + 1]
        target_entity_h = target_entity_seq_full[:, step + 1 : step + p + 1]
        target_entity_mask_h = target_entity_mask_seq_full[:, step + 1 : step + p + 1]
        ...
        # Next rollout state is previous predicted latent z_hat.
        z = pred_h
...
def main() -> None:
    ...
    saved_config = vars(args) | arch | {
        ...
        "training_regime": "markov_rollout_rnn_memory_enemy_visibility_sigreg_warmup",
        "enemy_visibility_mask": args.enemy_visibility_mask,
        "enemy_sight_range": args.enemy_sight_range,
    ...
    print(
        "markov_rollout_rnn_visibility_mask "
