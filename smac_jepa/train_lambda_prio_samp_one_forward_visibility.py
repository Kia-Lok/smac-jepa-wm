...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Mask enemy entity tokens whose saved global-state position is not within "
            "SMACLite sight range of any alive ally. This is computed per state/window "
            "from map_width/map_height and does not require recollecting data."
        ),
    )
    parser.add_argument(
        "--enemy-sight-range",
        type=float,
        default=9.0,
        help="SMACLite ally observation sight range in map units. Default: 9.0",
    )
...
def base_losses_from_outputs(
    ...
    """
    Exact one-forward equivalent of SMACJEPA.loss(...), using an already-computed
    output dict from model.forward(batch).

    This mirrors smac_jepa/jepa.py exactly. Keeping this logic identical matters:
    a previous one-forward attempt changed the effective loss scale.
    """
...
def lambda_jepa_losses(
    ...
    """
    Compute the TD(lambda)-weighted prediction objective with ONE model.forward
    call per batch.

    The ordinary non-prediction losses are computed from the same forward output
    using base_losses_from_outputs(...), which mirrors SMACJEPA.loss(...) exactly.
    """
...
def main() -> None:
    ...
    if args.enemy_visibility_mask:
        print(
            f"enemy_visibility_mask enabled sight_range={args.enemy_sight_range}",
            flush=True,
        )
