...
from smac_jepa.train_jepa_blocker_fixed_experiments import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Fail if visibility masking is enabled but no hidden target "
            "entities are observed during evaluation."
        ),
    )
...
def evaluate_one_checkpoint(
    ...
    if (
        args.require_hidden_eval
        and dataset.enemy_visibility_mask
        and hidden_dynamic_count <= 0.0
    ):
        raise RuntimeError(
            "Evaluation found zero hidden target entities. Refusing to "
            "write a misleading result."
        )
