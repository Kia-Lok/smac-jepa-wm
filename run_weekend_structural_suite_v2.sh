...
TRAIN_MODULE="smac_jepa.train_jepa_weekend_structural_v2"
EVALUATOR="./eval_jepa_weekend_structural_v2.py"
...
PROBE_DIR="runs/weekend_structural_probe_decoders_v2"
EVAL_ROOT="runs/weekend_structural_eval_v2"
...
# Preflight.
python -m py_compile \
  smac_jepa/train_jepa_weekend_structural_v2.py \
  "$EVALUATOR" || exit 2
...
SMOKE_DIR="runs/weekend_structural_smoke_v2"
rm -rf "$SMOKE_DIR"
...
run_experiment exp20_clean_event \
  runs/rnn_seqmem_exp20_clean_event_no_memory_barlow_v2 1 \
...
run_experiment exp28_direct_action_fusion \
  runs/rnn_seqmem_exp28_direct_action_fusion_v2 1 \
  --direct-action-fusion \
  --direct-action-hidden-dim 256 || true

run_experiment exp29_combined_transition \
  runs/rnn_seqmem_exp29_combined_transition_v2 1 \
  --residual-state-decoder \
  --delta-loss-weight 0.05 \
  --event-dynamics-weight 1.0 \
...
run_experiment exp24_observation_dropout \
  runs/rnn_seqmem_exp24_observation_dropout_v2 1 \
  --enemy-observation-dropout 0.20 || true

run_experiment exp25_hidden_reconstruction \
  runs/rnn_seqmem_exp25_hidden_reconstruction_v2 1 \
  --enemy-observation-dropout 0.20 \
...
run_experiment exp30_combined_transition_seed2 \
  runs/rnn_seqmem_exp30_combined_transition_seed2_v2 2 \
  --residual-state-decoder \
  --delta-loss-weight 0.05 \
  --event-dynamics-weight 1.0 \
  --event-dynamics-threshold 0.01 \
