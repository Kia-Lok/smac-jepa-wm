...
import eval_jepa_weekend_structural_v2 as base


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Targeted hidden-belief evaluation for blocker-fixed SMAC-JEPA "
            "checkpoints. Reuses existing probe decoders and adds natural "
            "occlusion, controlled contiguous occlusion, time-since-seen, "
            "memory-ablation, last-seen baseline, reappearance, and action-"
            "space audit metrics."
        )
    )
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "Stop the natural-hidden pass after this many previously-seen "
            "hidden enemy target entity-times have been scored."
        ),
    )
    parser.add_argument(
        ...
        default=5000,
        help=(
            "Maximum number of strided batches scanned across the full eval "
            "split. Batches without hidden targets are skipped before GPU work."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=[1, 3, 5],
        help=(
            "Preferred contiguous artificial occlusion lengths. Samples are "
            "cycled across these lengths and fall back to a shorter valid span."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Prefer spans with a visible observation immediately after the "
            "occlusion. Fall back to valid spans without reappearance when "
            "needed so hidden-state coverage is not lost."
        ),
    )
...
def gather_rollout_targets(
    ...
    """Gather target t=start+h+1 from a [B,T,...] sequence."""
    ...
    if int(index.max().item()) >= sequence.shape[1]:
        raise RuntimeError(
            "Rollout target index exceeds sequence length: "
            f"max_index={int(index.max().item())} "
...
def last_seen_before_sequence(
    ...
    """
    Return last observed state, whether one exists, and steps since last seen,
    all immediately before each sequence time.

    For a first hidden step after observation, age is 1.
    """
...
def add_regression_groups(
    ...
    *,
    ...
    for age_name, entity_mask in age_groups.items():
        ...
        position_age_mask = position_mask * entity_mask.unsqueeze(-1)
        base.add_exact_regression_statistics(
            accumulator,
            prefixes,
    ...
    *,
    ...
    if not probe_path.is_file():
        raise FileNotFoundError(
            "Existing probe decoder is required but was not found: "
            f"{probe_path}"
        )
...
def action_space_audit(
    *,
    ...
    memory_n_actions = int(
        getattr(memory_module, "n_actions", tensor_width)
    )
    ...
    if tensor_width != memory_n_actions:
        raise RuntimeError(
            "Action tensor width does not equal action-conditioned memory "
            f"width: tensor={tensor_width} memory={memory_n_actions}"
        )
    ...
    return {
        "action_space_audit_status": status,
...
def natural_hidden_pass(
    *,
    ...
    maximum_items = (
        int(args.natural_hidden_max_scan_batches)
        * int(args.batch_size)
    )
    loader = DataLoader(
    ...
    for cpu_batch in loader:
        ...
        enemy_dynamic = feature_masks["enemy_dynamic"]
        hidden_seen_entity = (
        ...
        hidden_dynamic = (
            enemy_dynamic * hidden_seen_entity.unsqueeze(-1)
        )
        ...
        add_presence_counts(
            presence_sums,
            presence_prefixes,
            prefix="natural_hidden_native_presence",
        ...
        add_presence_counts(
            presence_sums,
            presence_prefixes,
            prefix="natural_hidden_zero_memory_presence",
            logits=zero_outputs["presence_logits"],
            target_presence=outputs["target_entity_mask"],
            entity_mask=belief_presence_selector,
            threshold=args.presence_threshold,
        )

        # Reappearance: visible now after at least one full hidden step.
        observation = batch["observation_mask_seq"].bool()
        reappearance_seq = (
            observation
        ...
        reappearance_dynamic = (
            enemy_dynamic * reappearance_entity.unsqueeze(-1)
        )
        ...
        scored_hidden_entity_times += int(
            hidden_seen_entity.sum().detach().cpu()
        )
        scored_presence_entity_times += int(
            belief_presence_selector.sum().detach().cpu()
        )
        scored_reappearance_entity_times += int(
            reappearance_entity.sum().detach().cpu()
        )

        if (
            scored_hidden_entity_times
            >= int(args.natural_hidden_target_entity_times)
        ):
            break
    ...
    metadata = {
        ...
        "natural_hidden_presence_entity_times": (
            scored_presence_entity_times
        ),
...
def controlled_occlusion_schedule(
    ...
    *,
    ...
    """
    Select at most one visible enemy and one contiguous span per sample.

    Selected spans contain only originally visible, present, valid enemy
    targets. Spans with a visible reappearance step are preferred.
    """
    observation = batch["observation_mask_seq"].bool()
    target = batch.get(
        "target_entity_mask_seq",
    ...
    cleaned_spans = sorted(
        {int(value) for value in spans if int(value) >= 1}
    )
    ...
    if episode_index is None:
        ...
        episode_ids = [
            int(value)
            for value in episode_index.reshape(-1).tolist()
        ]

    for sample_index in range(batch_size):
        desired = cleaned_spans[
            (episode_ids[sample_index] + int(seed))
            % len(cleaned_spans)
        ]
        span_order = [desired] + [
            value
...
def make_masked_batch(
    ...
    result = dict(batch)
    result["observation_mask_seq"] = (
        batch["observation_mask_seq"].clone()
    )
    result["observation_mask_seq"][selected] = 0

    # Zero token values as a second leakage barrier. The encoder should already
    # respect observation_mask_seq, but hidden evaluation must not rely on that
    # assumption alone.
    result["entity_seq"] = batch["entity_seq"].clone()
    result["entity_seq"][
        selected.unsqueeze(-1).expand_as(result["entity_seq"])
    ] = 0
    return result
...
def controlled_occlusion_pass(
    *,
    ...
    maximum_items = (
        int(args.controlled_occlusion_max_batches)
        * int(args.batch_size)
    )
    loader = DataLoader(
        strided_subset(dataset, maximum_items=maximum_items),
    ...
    for cpu_batch in loader:
        if evaluated_batches >= int(
            args.controlled_occlusion_max_batches
        ):
            break
        ...
        (
            ...
            prefer_reappearance=bool(
                args.controlled_prefer_reappearance
            ),
        )
        if not records:
            continue
        ...
        for record in records:
            length_key = str(int(record["length"]))
            selected_span_histogram[length_key] = (
                selected_span_histogram.get(length_key, 0) + 1
            )
        ...
        with torch.no_grad(), autocast_context(device, amp_enabled):
            ...
            masked_probe, _ = base.decode_with_probe(
                masked_outputs,
                probe,
            )
            unmasked_probe, _ = base.decode_with_probe(
                unmasked_outputs,
                probe,
            )
        ...
        full_sequence = masked_batch.get(
            "target_entity_seq",
            masked_batch["entity_seq"],
        )
        last_seen_before, _, _ = last_seen_before_sequence(
            full_sequence,
            masked_batch["observation_mask_seq"],
        )
        last_seen_rollout = gather_rollout_targets(
            last_seen_before,
            rollout_window,
        ...
        families = (
            (
                "controlled_masked_native",
                masked_outputs["decoded"],
            ),
            (
                "controlled_zero_memory",
                zero_outputs["decoded"],
            ),
            (
                "controlled_unmasked_native",
                unmasked_outputs["decoded"],
            ),
            (
                "controlled_masked_probe_rollout",
                masked_probe,
            ),
            (
                "controlled_unmasked_probe_rollout",
                unmasked_probe,
            ),
            (
                "controlled_last_seen",
                last_seen_rollout,
            ),
        )
        for prefix, prediction in families:
            ...
            for length in sorted(
                ...
                length_entity = span_rollout == length
                length_dynamic = (
                    dynamic * length_entity.unsqueeze(-1)
                )
                base.add_exact_regression_statistics(
                    regression_sums,
                    regression_prefixes,
        ...
        add_presence_counts(
            presence_sums,
            presence_prefixes,
            prefix="controlled_masked_native_presence",
            logits=masked_outputs["presence_logits"],
            target_presence=masked_outputs["target_entity_mask"],
            entity_mask=selected_entity,
            threshold=args.presence_threshold,
        )
        add_presence_counts(
            presence_sums,
            presence_prefixes,
            prefix="controlled_zero_memory_presence",
            logits=zero_outputs["presence_logits"],
            target_presence=masked_outputs["target_entity_mask"],
            entity_mask=selected_entity,
            threshold=args.presence_threshold,
        ...
        add_presence_counts(
            presence_sums,
            presence_prefixes,
            prefix="controlled_unmasked_native_presence",
            logits=unmasked_outputs["presence_logits"],
            target_presence=masked_outputs["target_entity_mask"],
            entity_mask=selected_entity,
            threshold=args.presence_threshold,
        )
        ...
        scored_entity_times += int(
            selected_entity.sum().detach().cpu()
        )
        scored_reappearance_entity_times += int(
            reappearance_entity.sum().detach().cpu()
        )
        ...
        if (
            scored_entity_times
            >= int(args.controlled_occlusion_target_entity_times)
        ):
            break
    ...
    if masked is not None and unmasked is not None:
        metrics["controlled_occlusion_cost_dynamic_mae"] = (
            masked - unmasked
        )
    ...
    metadata = {
        ...
        "controlled_selected_with_reappearance": (
            selected_with_reappearance
        ),
...
def evaluate_checkpoint(
    ...
    memory_module = base.build_memory_module(
        checkpoint,
        dataset,
        device,
    )
    ...
    action_audit = action_space_audit(
        checkpoint=checkpoint,
        dataset=dataset,
    ...
    row: dict[str, Any] = {
        ...
        "hidden_belief_eval_version": 3,
        "natural_hidden_eval_enabled": bool(
            args.natural_hidden_eval
        ),
        ...
        "controlled_occlusion_spans": [
            int(value)
            for value in args.controlled_occlusion_spans
        ],
    }
    row.update(probe_info)
    row.update(action_audit)
...
def main() -> None:
    ...
    if args.natural_hidden_max_scan_batches < 1:
        raise SystemExit(
            "--natural-hidden-max-scan-batches must be positive"
        )
    ...
    amp_enabled = bool(args.amp and device.type == "cuda")
    print(
        f"hidden_belief_eval device={device} amp={amp_enabled} "
