...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Exp31--Exp33 JEPA checkpoints with padding-aware decoded-state, action-sensitivity, "
    ...
    *,
    ...
    for start in range(rollout_window):
        for step in range(horizon):
            ...
            # action_seq is one-hot encoded with shape
            # [B, agents, n_actions], while action_mask_seq has shape
            # [B, agents]. Convert one-hot vectors to discrete action IDs
            # before applying the attack-action threshold.
            if actions.dim() == valid_actions.dim() + 1:
                action_ids = actions.argmax(dim=-1)
            elif actions.dim() == valid_actions.dim():
                action_ids = actions.long()
            else:
                raise RuntimeError(
                    "Unexpected action tensor shapes: "
                    f"actions={tuple(actions.shape)} "
                    f"mask={tuple(valid_actions.shape)}"
                )

            attack_like = (
                (action_ids >= int(attack_action_min))
                & valid_actions
            ).any(dim=-1)
...
def build_memory_module(
    ...
    if bool(cfg.get("direct_action_fusion", False)):
        metadata = checkpoint.get("metadata", {})
        fusion = DirectOrderedActionFusion(
            n_actions=int(
                metadata.get("n_actions", dataset.metadata.n_actions)
            ),
            max_agents=int(
            ...
            max_entities=(
                int(metadata.get("max_agents", dataset.metadata.max_agents))
                + int(metadata.get("max_enemies", dataset.metadata.max_enemies))
            ),
            latent_dim=latent_dim,
            hidden_dim=int(cfg.get("direct_action_hidden_dim", 256)),
        ).to(device)
        state = checkpoint.get("action_fusion_module_state")
        if state is None:
            raise RuntimeError(
                "Direct-action-fusion checkpoint is missing module state"
            )
    ...
    memory_module.residual_state_decoder = bool(
        cfg.get("residual_state_decoder", False)
    )
    memory_module.eval()
...
def rollout_outputs(
    ...
    *,
    ...
    for start_idx in range(p):
        ...
        for step in range(h):
            ...
            if action_fusion_module is not None:
                pred_raw = pred_raw + action_fusion_module(
                    action_h[:, 0],
                    action_mask_h[:, 0],
                    entities=entities,
    ...
    decoded = (
        ...
        if bool(
            getattr(memory_module, "residual_state_decoder", False)
        )
        else decoded_raw
    )
